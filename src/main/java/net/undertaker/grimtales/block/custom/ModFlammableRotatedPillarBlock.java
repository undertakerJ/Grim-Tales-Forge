...
public class ModFlammableRotatedPillarBlock extends RotatedPillarBlock {
  ...
  public @Nullable BlockState getToolModifiedState(
    ...
    if (context.getItemInHand().getItem() instanceof AxeItem) {
      if (state.is(ModBlocks.VELESCIL_LOG.get())) {
        return ModBlocks.STRIPPED_VELESCIL_LOG
            .get()
            .defaultBlockState()
            .setValue(AXIS, state.getValue(AXIS));
      }
      if (state.is(ModBlocks.VELESCIL_WOOD.get())) {
        return ModBlocks.STRIPPED_VELESCIL_WOOD
            .get()
            .defaultBlockState()
            .setValue(AXIS, state.getValue(AXIS));
      }
